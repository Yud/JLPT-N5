...
- **WaniKani SRS** (`#/wanikani-srs`) — pick an SRS stage (Apprentice I … Guru II … Burned) and see every item you have at it, across all WaniKani levels, with when each is next up for review, sortable by next review. Fetched live from WaniKani on each load (`GET /api/wanikani/srs-items`, same `WANIKANI_API_KEY` secret as the sync), so it's never stale.
