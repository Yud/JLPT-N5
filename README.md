...
- **WaniKani levels** (`#/wanikani-levels`) — pick a WaniKani level and see every radical, kanji and vocab item in it with its SRS stage and when it's next up for review, sortable by next review. Fetched live from WaniKani on each load (`GET /api/wanikani/level`, same `WANIKANI_API_KEY` secret as the sync), so it's never stale.
